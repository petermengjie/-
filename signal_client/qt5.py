...
import requests
import json
from need import seq2seq_test_client
from need import covn_LSTM_test_client
...
class CameraPageWindow(QWidget, Ui_CameraPage):
    ...
    def __init__(self, parent=None):
        ...
        self.timer_net.start(2000)  # 3秒后加载默认网络
    ...
    def get_header(self):
        if not os.path.exists('../.session'):
            try:
                session = requests.get(self.url + '/getid', timeout=2).text
            ...
            session = session if session else 'default'  # 防止空文件
    ...
    def playAudio(self):
        if self.player and self.player.state() == QtMultimedia.QMediaPlayer.PlayingState:
            ...
            try:
                assert self.out_words
            except:
                return
            if self.words != self.out_words:
                self.words = self.out_words
                audio_name = self.api.get_auido(self.out_words)
    ...
    def set_net(self, net_id):
        try:
            res = requests.get(self.url + '/set?net=%d' % net_id, headers=self.headers, timeout=5)
        except:
            QMessageBox.information(None, 'Alert', '网络错误!')
            return 1
        result = json.loads(res.text)['code']
        if result != 0:
            QMessageBox.information(None, 'Alert', '服务器响应出错!')
            return 1
        else:
            return 0
    ...
    def loadNet(self):
        ...
        if self.btn1.isChecked():
            res = self.set_net(self.nets[0])
            text = self.btn1.text()
        else:
            res = self.set_net(self.nets[1])
            text = self.btn2.text()
        if res == 0:
            self.textRegion.setText('模型%s载入完成' % text)
            self.textRegion.repaint()
        else:
            return
    ...
    def inputNet(self):
        ...
        read_images = seq2seq_test_client.read_images if self.btn1.isChecked() else covn_LSTM_test_client.read_images
        if len(list(filter(lambda f: f.endswith(".jpg"), os.listdir(self.dirname)))) >= sample_duration:
            ...
            self.textRegion.repaint()
            try:
                self.out_words = read_images(self.dirname, self.url ,self.headers)
            except:
                QMessageBox.information(None, 'Alert', '网络错误!')
                return
            self.cameraLabel.setVisible(True)
